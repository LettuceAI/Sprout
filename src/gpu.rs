...
unsafe fn enumerate_vulkan() -> anyhow::Result<Vec<GpuDevice>> { unsafe {
...
unsafe fn device_supports_budget(instance: &ash::Instance, pd: vk::PhysicalDevice) -> bool { unsafe {
    let Ok(exts) = instance.enumerate_device_extension_properties(pd) else {
        return false;
    };
    exts.iter().any(|ext| {
        CStr::from_ptr(ext.extension_name.as_ptr()) == ash::ext::memory_budget::NAME
    })
}}
